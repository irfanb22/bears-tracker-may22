...
export function Dashboard() {
  ...
  useEffect(() => {
    const loadDisplayName = async () => {
      ...
      if (!error) {
        setDisplayNameDraft((data?.display_name || user.email?.split('@')[0] || '').trim());
      }
    };
            ...
            <div className="w-full max-w-sm rounded-lg border border-slate-200 bg-slate-50 p-3">
              ...
              <div className="mt-2 flex items-center gap-2">
                <input
                  ...
                  className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-800 focus:border-bears-orange focus:outline-none focus:ring-1 focus:ring-bears-orange"
                />
                <button
                  type="button"
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 sm:max-w-md">
              ...
              <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
                ...
                <p className="text-[11px] font-semibold text-slate-500">
                  {seasonMetrics.correctCount}/{seasonMetrics.resolvedCount} resolved correct
                </p>
              </div>
            </div>
              ...
              <>
                        ...
                        return (
                          <tr key={prediction.id} className="align-top text-sm shadow-[0_1px_0_rgba(15,23,42,0.05)]">
                            ...
                            <td className="border border-r-0 border-l-0 border-slate-200 bg-slate-50/40 py-3 pr-3">
                              <span className="font-semibold text-bears-navy">{prediction.prediction}</span>
                              <span className="ml-1 text-xs font-medium capitalize text-slate-500">({prediction.confidence})</span>
                            </td>
                            <td className="border border-r-0 border-l-0 border-slate-200 bg-slate-50/40 py-3 pr-3">
                            ...
                            <td className="border border-r-0 border-l-0 border-slate-200 bg-slate-50/40 py-3 pr-3 text-xs font-medium text-slate-500">
                ...
                <div className="mt-5 space-y-3 md:hidden">
                  {filteredPredictions.map((prediction) => {
                    ...
                    return (
                      <article key={prediction.id} className="rounded-lg border border-slate-200 p-3">
                        ...
                        <p className="mt-2 text-sm font-semibold text-bears-navy">
                          {prediction.prediction} <span className="text-xs font-medium capitalize text-slate-500">({prediction.confidence})</span>
                        </p>
                        {question.correct_answer?.trim() && (
